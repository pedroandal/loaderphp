<?php
    ...
    class Loader{
        ...
        * @property-write
        */
        public function disableCache() { $this->use_cache = false; }
        ...
        {   
            switch(gettype($views)){
                case 'array':
                    ...
                    break;
                case 'string':
                    if(empty($name)) $this->views[] = $views;
        ...
        */
        ...
        {                                    
            ...
            if(!$this->use_cache || $this->generatePage()) {  
                ...
                $this->processIfElse();
                $this->body_content = preg_replace("/<\/php:(if|else|for)\s*>/i", "<?php } ?>", $this->body_content);
            ...
            if($this->debug) $this->debug($debug); //faz debug ao modelo
        ...
        {        
            $meta=array();
            $meta['Content-Type'] = "text/html; charset={$this->charset}"; //text encoding
            ...
            $dynamic_html = "";  
            //$count = preg_match_all('/<!--\[[^\]]*\]>[^>]*(?(?<!\<!\[endif\]--)>[^>]*)*>/i', $this->body_content, $dynamic, PREG_OFFSET_CAPTURE);
            ...
            //$count = preg_match_all('/<title\s*[^>]*>[^>]*(?(?<!\<\/title)>[^>]*)*>/i', $this->body_content, $title, PREG_OFFSET_CAPTURE);
...
/*
            $count = preg_match_all('/<link.*?[^-\?]>/si', $this->body_content, $link, PREG_OFFSET_CAPTURE);    
            if($count > 0) {
                $link = $link[0];
                for($i = $count-1; $i >= 0; $i--) {
                    $this->body_content = substr_replace($this->body_content, "", $link[$i][1], strlen($link[$i][0]));
                    $this->head_content = "{$link[$i][0]}{$this->head_content}";
                }
            }
                                                                                                                                        
            $count = preg_match_all('/<meta.*?[^-\?]>/si', $this->body_content, $meta, PREG_OFFSET_CAPTURE);
            if($count > 0) {
                $meta = $meta[0];
                for($i = $count-1; $i >= 0; $i--) {
...
*/
            ...
            //$count = preg_match_all('/<style\s*[^>]*>[^>]*(?(?<!\<\/style)>[^>]*)*>/i', $this->body_content, $style, PREG_OFFSET_CAPTURE);
            ...
            if($count > 0) {           
                for($i = $count-1; $i > -1; $i--) {
                    ...
                    //$stripedStyle = str_replace(array("</style>", $style[1][$i][0]), "", $style[0][$i][0]);
            ...
            //$count = preg_match_all('/<script\s*(?:[^>]*(?(?<=-|\?)>[^>]*))*(?:[^>]*(?(?<!\/|\<\/script)>[^>]*))*>/i', $this->body_content, $script, PREG_OFFSET_CAPTURE);
        ...
        {
            while(preg_match("/<php:view([^>]*)>/i", $this->body_content, $view, PREG_OFFSET_CAPTURE))
            {
...
*/
        ...
        {              
            ...
            while(preg_match("/<php:(if|else)([^>]*)>/i", $this->body_content, $ifelse)) {
              ...
              /*      }   */                        
                ...
                //$ifelse[2] = "$ifTestCode {$ifelse[1]} ({$ifelse[2]})";
                if($ifelse[1]=="else")
        ...
        */
        ...
        {
            if(!empty($var))
                $this->debugMsg = sprintf("%s<fieldset class='loaderphp_debug'>%s<textarea class='loaderphp_debug' rows='15'>%s</textarea></fieldset>",
    ...
    class Database{               
        ...
        */
        public function SQlite($filename, $key=NULL){
            ...
            switch($this->driver){
                ...
                default:
                    die("SQlite Error: Can't connect to database!<br>{$this->errorLog}");
            }
        }
        ...
        */
        function execute($sql, $params=NULL)
        {
            ...
            if($this->debug)
                echo "<span style='display:block; width:100%'>Execute: ", $sql, "</span>";
            switch($this->driver){
                case 'sqlite' :
        ...
        */
        ...
        private function InterBase_Firebird($host, $db, $usr, $pwd){
            ...
            if($this->driver == "interbase")
                $this->conn = call_user_func_array($this->persistentConn ? 'ibase_pconnect' : 'ibase_connect', array("$host:$db", $usr, $pwd));
            else 

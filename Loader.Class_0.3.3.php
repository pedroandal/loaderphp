        ...
        public function loadResources($filepath, $section=null)
        {
            if(empty($filepath) && $this->debug)
                ...
                if(is_file($resPath)){       
                    ...
                    return is_array($resources) && count($resources)>0 ? array_map('htmlentities', $resources) : false;  

                } else if($this->debug)
                    $this->warning("Wrong resources path or invalid file '$resPath'!\n");
            }
            return false;
        }
            ...
            switch($this->driver){
                ...
                case "mysql":
                    if(extension_loaded("mysql")){
                        $this->driver = "mysql";
                        if(($this->conn = mysql_connect("$host:$port", $username, $password)) != FALSE) { 
        ...
        */
        ...
        private function InterBase_Firebird($host, $db, $usr, $pwd){
            ...
            if($this->driver == "interbase")
                $this->conn = ibase_connect("$host:$db", $usr, $pwd);
             else 

<?php                                         
    ...
    ini_set("error_prepend_string", "\n<style type='text/css'>fieldset.loaderphp_error br{display:none;}</style><fieldset class='loaderphp_error' style='border:thin solid red;'><legend style='color:red;'>ERROR</legend>");
    ...
    class Loader{                            
        ...
        */
        ...
        public function useCache($bool) { $this->use_cache = $bool; } 
        ...
        {                                                                  
            ...
            if($method == 'get') $request = array_merge($_POST, $_GET);
            else if($method == 'post') $request = array_merge($_GET, $_POST); 

                foreach($request as $key=>$val)
                $this->vars[preg_replace("/[^a-zA-Z0-9_]/", "_", $key)] = $val;   
            ...
            if(isset($_SESSION))
            {
                ...
                foreach($_SESSION as $key=>$val)
                    $tmp[preg_replace("/[^a-zA-Z0-9_]/", "_", $key)] = $val;   
                $this->vars["session"] = $tmp;
            }
        ...
        */                             
        public function setMainView($view) {
            if(!isset($this->mainView)){ 
                if(!is_string($view)) $this->error(500, "MasterPage must be a string.\n $loader->setMasterPage(string pathname);");
                if(!file_exists(realpath($view))) $this->error(404, "MasterPage file doesn't exist.\nPlease referer a valid file.");
                $this->mainView = $view; 
        ...
        public function loadResources($filepath, $section=null)
        {
            if(empty($filepath) && $this->debug)
                ...
                if(is_file($resPath)){       
                    ...
                    return is_array($resources) && count($resources)>0 ? new ArrayObject($resources, ArrayObject::ARRAY_AS_PROPS) : false;  
        ...
        */
        ...
        {            
            ...
            while(empty($this->body_content)) { //retrieve views until last one
                count($this->views) > 0 || $this->error(404, "Nothing to show. Add views to process!");
                $this->body_content = $this->loadViewPart(array_shift($this->views));
            }
            $this->manageViews(); //load viewParts  
            $this->body_content = trim(preg_replace( //clean html view
            ...
            while(($head=$this->getAndCleanText("<script ", "</script>"))!==false){               
                ...
                {                                               
                    if(!preg_match("/^(ht|f)tps?:\/\//", $script[1]) && $script[1][0] != '/')    
                        $script[1] = str_replace(array($_SERVER['DOCUMENT_ROOT'], '\\'), array('', '/'), realpath($script[1]));
        ...
        private function writeResponse($file="")
        {
            ...
            {                                           
                ...
                else if(strstr($_SERVER['HTTP_ACCEPT_ENCODING'],"deflate")) {
        ...
        private function getAndCleanText($initStr, $endStr, $cleanTags=false) {  
            if(is_string($initStr) && is_string($endStr) && strlen($initStr)>0 && strlen($endStr)>0 && ($posI = stripos($this->body_content, $initStr))!== false) {
                ...
                if($posE > $posI) {
                    ...
                    $this->body_content = substr_replace($this->body_content, "", $posI, $posE-$posI+strlen($endStr)); 
                    return $retStr;
                }
            }                     
            return false;
        ...
        */
        ...
        { 
            ...
            {
                $posE = stripos($this->body_content, $endStr, $posI);
                if($posE > $posI) return substr($this->body_content, $posI, $posE-$posI+strlen($endStr)); 
            }                     
            return false;
        }  
        ...
        private function processFor()
        {
            while(preg_match("/<php:for\s+([^>]+)>/i", $this->body_content, $for))
            {
                if($for[1]!="")
                {
                    $forVars=preg_split("/\s(is|as)\s/", $for[1]);   
                    $forVars[0]=trim($forVars[0], " \$");
                    ...
                    $forNewVar = array();
                    for($forIti = 0; $forIti < $forVarsItems; $forIti++){
                        $forNewVar[$forIti] = "\$??{$forVars[0]}_{$forOldVar[$forIti]}";
                        $forOldVar[$forIti] = "\${$forOldVar[$forIti]}";
                    }  
                    $forNewHtml = str_replace($forOldVar, $forNewVar, $forHtml);
        ...
        {                                                                                                                                                                            
            ...
            {  
                ...
                $ifelse[2] = rtrim($ifelse[2]," \/");
                if(!empty($ifelse[2])) {
                    $ifelse[2] = "({$ifelse[2]}";
                    $this->processVars($ifelse[2]);
                    $this->processCondition($ifelse[2]);
                        
                    $replace = preg_replace("/\\\$this->vars[\\\"[a-zA-Z0-9_]+\\\"]/", "isset($0) ? $0 : FALSE", $replace);
                    $repIsset = preg_replace("/isset\s*\(\s*isset/", "isset", $replace);
                    if($replace != $repIsset)
                        $replace = "$repIsset if{$ifelse[2]}";
                    else
                        $replace = "$replace if{$ifelse[2]})";
                }
                
                
                $this->body_content = str_replace($ifelse[0], "<?php $replace { ?>", $this->body_content);
            }                                  
        }

        private function processCondition(&$condition)
        {
            $arrCond = preg_split("/\s(and|or)\s/", $condition);
            if(($arrLen = count($arrCond)) > 0)
                for($i=0; $i<$arrLen; $i++)
                {   
                    $rep = preg_replace(array("/<\?php\s(.+\s)?echo\s/i", "/;\s\?>/i"), "", $arrCond[$i]);
                    $condition = str_replace($arrCond[$i], $rep, $condition);
                }
            $condition = preg_replace(
                array("/\s+eq\s+/", "/\s+neq\s+/", "/\s+lt\s+/", "/\s+lte\s+/", "/\s+gt\s+/", "/\s+gte\s+/", "/\s+and\s+/", "/\s+or\s+/"), 
                array(" == ", " != ", " < ", " <= ", " > ", " >= ", " && ", " || "), 
                $condition);
        }
        ...
        { /* Replacing ordinary variables */
            $vpos=0;
            while(preg_match("/([^\\\$])(\\\$([a-zA-Z0-9_\.\?]+))(}?)/i",$html, $var, NULL, $vpos)) 
            {
                if($var[3]=="" || $var[3]=="this" || substr($var[3], 0, 2)=="__") 
                {
                    $vpos = strpos($html, $var[0], $vpos) + strlen($var[0]);
                    continue;
                }

                if($var[1]=="{" && $var[4]=="}") $var[1]="";
                
                $replace = $this->processVar($var[3]);
                $replace = "{$var[1]}<?php $replace; ?>";
                $html = str_replace($var[0], $replace, $html);
                $vpos = strpos($html, $replace, $vpos) + strlen($replace);
            }
        }
        ...
        private function processVar($varName)
        {
            if(substr($varName, 0, 2)!="??")
                $replace = 'if(array_key_exists("%1$s", $this->vars)) echo $this->vars["%1$s"]%2$s';
            else{
                $replace = 'echo $__%1$s%2$s';
        ...
        */
        ...
        {                       
            ...
            {                               
                ...
                $script="";
                foreach($this->headers['script'] as $s) $script = "$script<script type='text/javascript' src='$s'></script>";                                                                                                                                                      
                $content = sprintf("<!DOCTYPE html><html><head>%s</head><body>%s%s%s%s</body></html>\r\n",    
                    $this->head_content, 
                    $this->debugMsg, 
                    $this->body_content,
                    $script,
        ...
        */
        ...
        private function error($code, $msg)
        {
            ...
            die("<!DOCTYPE html><html><head><meta http-equiv='Content-Type' content='text/html; charset=UTF-8' /><title>$code {$errors[$code]}</title></head><body><center><h4>$msg</h4>\n<hr/>{$this->name}/{$this->version}</center></body></html>");   
        }     
    ...
    class Database{
        ...
        */
        function query($sql, $params=NULL)
        {
            ...
            switch($this->driver){
                case 'sqlite' :
                    $this->result = sqlite_unbuffered_query($this->conn, $sql);   
                    break;
                case 'sqlite3' :
                ...
                case 'interbase':
                    ...
                    break;
                case 'mysqli':
                    if(mysqli_real_query($this->conn, $sql) && mysqli_field_count($this->conn)) {
                        $this->result = mysqli_store_result($this->conn);
                    }
                    break;
                case 'mysql':
